...
func (d *descheduler) runProfiles(ctx context.Context, client clientset.Interface, nodes []*v1.Node, podEvictor *evictions.PodEvictor) {
	...
	defer span.End()
	var profileRunners []profileRunner // 记录了所有要执行的 Profile 的名字和对应的执行函数。
	for _, profile := range d.deschedulerPolicy.Profiles {
		currProfile, err := frameworkprofile.NewProfile( // NewProfile 会创建一个 Profile（里面是一个插件）对象。
			profile,                       // Profile 的 value。
			pluginregistry.PluginRegistry, // 本质上是一个 map
...
func RunDeschedulerStrategies(ctx context.Context, rs *options.DeschedulerServer, deschedulerPolicy *api.DeschedulerPolicy, evictionPolicyGroupVersion string) error {
	...
	// 创建一个用于监视 Kubernetes 节点资源对象变化的 SharedInformerFactory，并从中获取一个 NodeLister 对象，以便后续可以使用 NodeLister 对象来检索节点信息。
	sharedInformerFactory := informers.NewSharedInformerFactoryWithOptions(rs.Client, 0, informers.WithTransform(trimManagedFields))
	nodeLister := sharedInformerFactory.Core().V1().Nodes().Lister()

	// 在 Kubernetes 中，Informers 是一种用于监视 Kubernetes 资源对象变化的机制。它们是 Kubernetes 客户端库中的一部分，用于创建和管理资源对象的事件监听器。

	var nodeSelector string
	if deschedulerPolicy.NodeSelector != nil {
		nodeSelector = *deschedulerPolicy.NodeSelector // yaml 文件中的 NodeSelector 字段
	}
	...
	// wait.Until 会在指定的时间间隔内执行指定的函数，直到函数返回错误或者上下文被取消。
